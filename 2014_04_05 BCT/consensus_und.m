function [ciu, Q] = consensus_und(d,tau,reps)
...
%   modified again on 11/2014 by Davy vanderweyen

n = length(d); flg = 1;
Q = [];
reps=reps/10;    % to make the less partitions than modularity runs
while flg == 1
    
    flg = 0;
    dt = d.*(d >= tau).*~eye(n);   %dt is thresholded agreement matrix
    if nnz(dt) == 0
        ciu = (1:n)';
    else
        ci = zeros(n,reps);
        for iter = 1:reps
            [ci(:,iter), Q2] = modularity_louvain_und_sign(dt,'pos');
            Q = [Q Q2];
        end
